...
<head>
  ...
  <title>Опрос</title>
  <meta property="og:title" content="Пройди опрос и получи подарок! 🎁">
  <meta property="og:description" content="Ответь на несколько вопросов и получи бонус">
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .option.selected .option-radio::after{content:'✓';color:#000;font-size:12px;font-weight:bold}
...
window.onerror = function(msg, url, line) {
  ...
  if (app) {
    app.innerHTML = '<div style="padding:40px;text-align:center;color:#ff6b6b;"><h2>❌ Ошибка JavaScript</h2><p>' + msg + '</p><p style="font-size:12px;color:#888;">Line: ' + line + '</p><button onclick="location.reload()" style="margin-top:20px;padding:10px 20px;background:#FFD700;border:none;border-radius:8px;cursor:pointer;">🔄 Перезагрузить</button></div>';
  }
...
const CONFIG = {
  ...
  BASE_URL: window.location.origin // Автоматически определяет домен
};
...
// Демо-опрос (потом будет из базы)
const DEMO_SURVEY = {
  id: 'demo-1',
  ...
  questions: [
    {
      id: 1,
      text: 'Сколько времени вы готовы уделять дополнительному заработку?',
      options: ['1-2 часа в день', '3-4 часа в день', 'Полный рабочий день', 'Пока не определился']
    },
    {
      id: 2,
      text: 'Какой опыт работы в интернете у вас есть?',
      options: ['Никакого', 'Пробовал фриланс', 'Есть небольшой опыт', 'Работаю онлайн давно']
    },
    {
      id: 3,
      text: 'Что для вас важнее в работе?',
      options: ['Стабильный доход', 'Свободный график', 'Возможность роста', 'Работа из дома']
    },
    {
      id: 4,
      text: 'Какой уровень дохода вас интересует?',
      options: ['$300-500/мес', '$500-1000/мес', '$1000-3000/мес', 'Больше $3000/мес']
    },
    {
      id: 5,
      text: 'Готовы ли вы обучаться новому?',
      options: ['Да, готов вкладывать время', 'Да, если это несложно', 'Только если быстро окупится', 'Хочу готовое решение']
    }
  ]
};
...
class SurveyApp {
  constructor() {
    ...
    if (!this.container) {
      ...
      document.body.innerHTML = '<div style="padding:40px;text-align:center;color:#ff6b6b;"><h2>❌ Ошибка</h2><p>Container не найден</p></div>';
      return;
    }
    
    console.log('✅ SurveyApp starting...');
    ...
    this.init().catch(err => {
      ...
      this.state.error = err.message || 'Ошибка инициализации';
  ...
  async init() {
    ...
    // Проверить, есть ли сохранённый респондент
    const savedRespondent = localStorage.getItem('survey_respondent_' + surveyId);
    
    try {
      // Загрузить опрос из Supabase
      let survey = null;
      
      // Проверяем - это локальный опрос или из базы?
      if (surveyId.startsWith('local_')) {
        // Локальный опрос из localStorage
        ...
        console.log('📋 Local survey:', survey ? 'found' : 'not found');
      } else if (surveyId === 'demo-1') {
        // Демо опрос
        survey = DEMO_SURVEY;
        console.log('📋 Demo survey loaded');
      } else {
        // Загрузка из Supabase - сначала по id, потом по short_code
        ...
        console.log('📋 By ID result:', data ? 'found' : 'not found', error?.message || '');
        
        // Если не нашли по id, пробуем по short_code
        if (!data) {
          const result = await supabaseClient
            ...
            .maybeSingle();
          data = result.data;
          console.log('📋 By short_code result:', data ? 'found' : 'not found');
        }
      ...
      // Парсим questions если это строка
      ...
      // Нормализуем данные опроса
      this.state.survey = {
        id: survey.id,
        short_code: survey.short_code || null,
        owner_gw_id: survey.owner_gw_id || null, // ДЛЯ СОХРАНЕНИЯ КОНТАКТОВ!
        title: survey.title || 'Опрос',
        description: survey.description || '',
        icon: survey.icon || '📋',
        reward_text: survey.reward_text || 'Подарок за прохождение',
        referral_reward: survey.referral_reward_text || survey.referral_reward || 'Бонус за друга',
        questions: questions,
        completions_count: survey.completions_count || 0
      };
      
      console.log('📋 Survey loaded:', this.state.survey.title, 'Owner:', this.state.survey.owner_gw_id);
      
      if (!questions || questions.length === 0) {
        throw new Error('Опрос не содержит вопросов');
      }
      ...
      this.state.error = e.message || 'Ошибка загрузки опроса';
      this.state.screen = 'error';
    }
  ...
  async saveRespondent(contactData) {
    ...
    try {
      // === ДЕДУПЛИКАЦИЯ ===
      // Проверяем не проходил ли уже этот контакт этот опрос
      const { data: existing } = await supabaseClient
        .from('survey_respondents')
        .select('id')
        .eq('survey_id', surveyId)
        .eq('contact', contactData.contact)
        .maybeSingle();
      
      if (existing) {
        console.log('⚠️ Дубликат контакта:', contactData.contact);
        // Просто показываем результат, не сохраняем дубликат
        this.state.respondent = existing;
        localStorage.setItem('survey_respondent_' + surveyId, JSON.stringify(existing));
        this.showNotification('Вы уже проходили этот опрос!', 'warning');
        return existing;
      }
      ...
      const respondentData = {
        ...
        push_consent: contactData.pushConsent || false,
      ...
      // Получаем owner_gw_id опроса для сохранения контакта
      ...
      // Сохраняем контакт в таблицу contacts для владельца опроса
      console.log('📧 Saving contact. Owner GW ID:', ownerGwId);
      ...
      if (ownerGwId) {
        ...
        // 1. Сохраняем владельцу опроса
        const ownerSaved = await this.saveContactToOwner(ownerGwId, baseContactData, 0);
        
        // 2. Распространяем вверх по рефералке (до OWNER)
        if (ownerSaved) {
          await this.distributeContactUpline(ownerGwId, {
            ...baseContactData,
            source: 'survey_viral' // Помечаем как вирусный контакт
          });
        }
      }
      
      // Увеличиваем счётчик завершений опроса
      ...
      // Обновить счётчик рефералов у пригласившего
  ...
  generateShareLink() {
    if (!this.state.respondent) return '';
    // Используем короткую ссылку /s/{short_code или id}
  ...
  // ═══════════════════════════════════════════════════════════
  // СПОНСОРСКАЯ СИСТЕМА - Распространение контактов вверх
  // ═══════════════════════════════════════════════════════════
  
  // Сохранить контакт конкретному владельцу с дедупликацией
  async saveContactToOwner(ownerId, contactData, level = 0) {
    ...
    try {
      // Проверяем дубликат - ищем по обоим вариантам ID
      const { data: existing } = await supabaseClient
        .from('contacts')
        .select('id')
      ...
      if (existing) {
        console.log(`⏭️ Level ${level}: Contact already exists for ${ownerId}`);
      ...
      const toSave = {
        ...contactData,
        owner_gw_id: rawId, // Сохраняем без GW префикса для единообразия
        source_level: level
      };
      
      await supabaseClient.from('contacts').insert(toSave);
      console.log(`✅ Level ${level}: Contact saved to ${ownerId}`);
      return true;
      
    } catch (e) {
      console.warn(`⚠️ Level ${level}: Error saving to ${ownerId}:`, e.message);
      return false;
    }
  }
  
  // Получить ID спонсора (реферера)
  async getSponsorId(userId) {
    ...
    try {
      ...
      console.warn('getSponsorId error:', e);
      return null;
    }
  }
  
  // Распространить контакт вверх по рефералке до OWNER
  async distributeContactUpline(startId, contactData, maxLevels = 10) {
    let currentId = startId;
    let level = 1;
    const distributed = [];
    
    console.log('📤 Starting upline distribution from:', startId);
    
    while (currentId && level <= maxLevels) {
      // Получаем спонсора
      const sponsorId = await this.getSponsorId(currentId);
      
      if (!sponsorId || sponsorId === currentId) {
        console.log(`🏁 Level ${level}: No more sponsors (reached top)`);
        break;
      }
      
      // Сохраняем контакт спонсору
      const saved = await this.saveContactToOwner(sponsorId, {
        ...contactData,
        source_details: JSON.stringify({
          ...JSON.parse(contactData.source_details || '{}'),
          via_user: currentId,
          upline_level: level
        })
      }, level);
      ...
      if (saved) {
        distributed.push({ id: sponsorId, level });
      } else {
        // Если дубликат - прекращаем распространение вверх
        // (если контакт уже есть у спонсора, то он уже есть и выше)
        console.log(`🛑 Level ${level}: Duplicate found, stopping distribution`);
        break;
      }
    ...
    console.log(`✅ Contact distributed to ${distributed.length} sponsors:`, distributed.map(d => d.id).join(' → '));
    return distributed;
  }
  ...
  showNotification(message, type = 'info') {
    ...
    notif.style.cssText = `
      ...
      ${type === 'warning' ? 'background: #ff9800; color: #000;' : 
        type === 'error' ? 'background: #f44336; color: #fff;' :
        type === 'success' ? 'background: #4CAF50; color: #fff;' :
  ...
  shareToTelegram() {
    const link = this.generateShareLink();
    const text = `🎁 Пройди опрос "${this.state.survey.title}" и получи подарок!\n\n`;
  ...
  shareToViber() {
    const link = this.generateShareLink();
    const text = `🎁 Пройди опрос и получи подарок! ${link}`;
  ...
  async submitContact(e) {
    ...
    const pushConsent = form.push_consent?.checked || false;
    ...
    if (type === 'phone' || type === 'whatsapp') {
      ...
      // Если начинается с 0 - добавляем +38
      if (contact.startsWith('0')) {
        contact = '+38' + contact;
      }
      // Если начинается с 38 без + - добавляем +
      else if (contact.startsWith('38') && !contact.startsWith('+')) {
        contact = '+' + contact;
      }
      // Если начинается с 8 (российский формат) - конвертируем в +7
      else if (contact.startsWith('8') && contact.length === 11) {
        contact = '+7' + contact.substring(1);
      }
      
      // Проверяем формат: должен начинаться с + и содержать 12-13 цифр
      const phoneRegex = /^\+\d{10,13}$/;
      if (!phoneRegex.test(contact)) {
        this.state.error = 'Неверный формат телефона. Используйте формат +380XXXXXXXXX';
        this.render();
        return;
      }
    }
    ...
    if (type === 'email') {
      ...
      if (!emailRegex.test(contact)) {
        this.state.error = 'Неверный формат email';
        this.render();
        return;
      }
    }
    ...
    // Валидация telegram
    if (type === 'telegram') {
      // Убираем @ в начале если есть, потом добавим
      if (contact.startsWith('@')) {
        contact = contact.substring(1);
      }
      // Проверяем что это валидный username
      if (contact.length < 3) {
        this.state.error = 'Telegram username должен быть минимум 3 символа';
        this.render();
        return;
      }
      // Добавляем @ обратно
    ...
    await this.saveRespondent({ name, contact, type, pushConsent });
  ...
  render() {
    ...
    console.log('🎨 Render screen:', screen, 'Survey:', survey ? survey.title : 'null');
    
    let html = '';
    
    // Защита от null survey для экранов которые требуют данные
        ...
        html = `
          <div class="card">
            ...
            <div style="margin-bottom:20px;color:var(--text-muted);font-size:13px;text-align:center">
              <p>📋 ${survey.questions.length} вопросов • ⏱️ ~2 минуты</p>
            </div>
          ...
          <div class="card">
            <div class="progress-container">
              <div class="progress-text">
                <span>Вопрос ${currentQuestion + 1} из ${survey.questions.length}</span>
            ...
            <div class="question">
              <div class="question-num">Вопрос ${currentQuestion + 1}</div>
            ...
            <div style="display:flex;gap:10px">
              ...
              <button class="btn btn-primary" style="flex:2" onclick="app.nextQuestion()" ${!currentAnswer ? 'disabled' : ''}>
                ${currentQuestion < survey.questions.length - 1 ? 'Далее →' : 'Завершить ✓'}
              </button>
            </div>
          </div>
        `;
        ...
        html = `
          <div class="card">
            <div class="header">
              ...
              <h1>Отлично!</h1>
              <p>Осталось заполнить контакт для получения подарка</p>
            </div>
            ...
            <form onsubmit="app.submitContact(event)">
              <div class="input-group">
                ...
                <input type="text" name="name" class="input" placeholder="Как вас зовут?" required>
              </div>
              ...
              <div class="input-group">
                ...
                <input type="text" name="contact" class="input" placeholder="@username, email или номер" required>
              </div>
              
              <div class="input-group" style="margin-top:15px">
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:13px;color:var(--text-muted)">
                  <input type="checkbox" name="push_consent" style="width:18px;height:18px;margin-top:2px;accent-color:var(--gold)">
                  <span>🔔 Согласен получать уведомления о новых предложениях и бонусах</span>
                </label>
              </div>
      ...
      case 'result':
        html = `
          <div class="card">
            ...
            <div class="gift-banner">
              <div class="gift-banner-icon">🎁</div>
              <h3>Ваш подарок готов!</h3>
              <p>${survey.reward_text}</p>
            </div>
            
            <div style="background:var(--bg-lighter);border-radius:12px;padding:20px;text-align:center;margin-bottom:20px">
              <div style="font-size:14px;color:var(--text-muted);margin-bottom:10px">Ваш уникальный ID</div>
            ...
            <div style="margin-top:15px;text-align:center;color:var(--text-muted);font-size:12px">
              💡 За каждого друга — дополнительный бонус!
            </div>
          </div>
        `;
        break;
        ...
        html = `
          <div class="card">
            <div class="share-section">
              ...
              <div class="share-title">Пригласите друзей!</div>
              ...
              <div class="share-buttons">
                ...
                <button class="share-btn copy" onclick="app.copyLink()" title="Копировать">
                  📋
                </button>
              </div>
            </div>
            ...
            ${referrals.length > 0 ? `
              <div class="referral-list">
                <h3 style="font-size:14px;color:var(--text-muted);margin-bottom:15px">Ваши приглашённые:</h3>
                ${referrals.map(r => `
            ...
            ${referrals.length >= 3 ? `
              <div class="reward-card">
                ...
                <p>Вы пригласили 3+ друзей. Ваш бонус активирован!</p>
              </div>
            ` : `
              <div style="margin-top:20px;text-align:center;color:var(--text-muted);font-size:12px">
                🎯 Пригласите ещё ${3 - referrals.length} друзей для получения награды
              </div>
            `}
          </div>
        `;
